...
class ResNet(nn.Module):
	...
	def __init__(self, block, layers, num_classes=1000):
		...
		# self.avgpool = nn.AvgPool2d(14)  # 池化2048*16*16，变成2048*1*1
		# self.fc = nn.Linear(512 * block.expansion, num_classes)
		self.avgpool = nn.AvgPool2d(8)  # 池化2048*16*8变成2048*2*1
	...
	def forward(self, x):
		...
		# print('in', x.shape)  # [bs, 2048, 16, 8]  16 = 256/16  8 = 128/16
		x = self.avgpool(x)
		# print('in2', x.shape)
		x = x.view(x.size(0), -1)
		x = self.fc(x)
...
def attempt_load(model_name, pretrained=False, **kwargs):
	mn = model_name
	if mn == 'resnet18':
		...
		raise NameError(f'model_name "{mn}" is not defined.')
